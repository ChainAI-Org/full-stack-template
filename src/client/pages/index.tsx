  ...
  return (
    <div className="space-y-16 md:space-y-24 py-12 md:py-20">
      ...
      <section className="text-center">
        <h1
          className="text-5xl md:text-7xl font-bold mb-6 h-14 md:h-20
                       bg-gradient-to-r from-brand-accent-blue via-brand-accent-cyan 
                       text-transparent bg-clip-text animate-gradient-x"
        >
          {appName}
        </h1>
        <p className="text-xl md:text-2xl dark:text-brand-dark-text-secondary text-brand-light-text-secondary mb-10 max-w-2xl mx-auto">
          {tagline}
        </p>
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
          {user ? (
            <Button href="/tasks" size="lg" variant="primary" className="text-brand-light-surface" rightIcon={<TrendingUp size={20} />}>
              Go to My Tasks
            </Button>
          ) : (
            <>
              <Button href="/signup" size="lg" variant="primary" className="text-brand-light-surface" rightIcon={<UserCircle size={20} />}>
                Get Started Free
              </Button>
              <Button href="/login" size="lg" variant="outline" className="">
                Log In
              </Button>
            </>
          )}
